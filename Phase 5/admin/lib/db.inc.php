...
function ierg4210_order_fetchAllByUsername($name)
{
    global $db;
    $db = ierg4210_DB();
    $q = $db->prepare("SELECT * FROM ORDERS WHERE USERNAME = ? ORDER BY UUID DESC LIMIT 5;");
    $q->bindParam(1, $name,PDO::PARAM_STR|PDO::PARAM_INPUT_OUTPUT,20);
    if ($q->execute())
        return $q->fetchAll();
}

function ierg4210_order_fetchAllByInvoiceDigest($invoice,$digest)
{
    global $db;
    $db = ierg4210_DB();
    $q = $db->prepare("SELECT * FROM ORDERS WHERE UUID = ? AND ORMSG = ? ;");
    $q->bindParam(1, $invoice,PDO::PARAM_INT|PDO::PARAM_INPUT_OUTPUT,20);
    $q->bindParam(2, $digest,PDO::PARAM_STR|PDO::PARAM_INPUT_OUTPUT);
    if ($q->execute())
        return $q->fetchAll();
}
